...
<div class="container mb-4">
    ...
    <form method="POST" action="{{route('socios.ativar')}}" >
        @csrf
        @method('PATCH')
        <button onclick="deactivateSocios()" class="btn btn-outline-primary mb-2">Desativar sócios com quotas por
            pagar</button>
    </form>
</div>
                    ...
                    <form method="POST" action="{{route('ativarDesativarSocio', $user->id)}}">
                        ...
                        <div class="form-group mb-2">
                            <select name="ativo" id="ativo" class="form-control" value="{{ $user->ativo}}">
                                ...
                                <option value="1" {{ old('ativo', strval($user->ativo)) == "1" ? 'selected' : '' }}>
                                    Ativo</option>
